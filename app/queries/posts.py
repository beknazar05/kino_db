...
#     return post
@db
def create_post(post_in: PostCreateSchema) -> None:
    post = Post.create(
        title=post_in.title,
...
@db 
def get_all_films() -> None:
    posts = Post.select(Post.title, Post.year)
    return [{'title': post.title,'year': post.year} for post in posts]
...
# TODO: dopisat crud dly postov 
# TODO: oznokomitsy s fastapi
